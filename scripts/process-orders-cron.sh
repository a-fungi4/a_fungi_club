...
# Process Square Orders Cron Job
# This script polls Square for new orders and sends them to Printful
...
if [ "$HTTP_CODE" -eq 200 ]; then
    ...
    if [ -n "$PROCESSED" ] && [ "$PROCESSED" -gt 0 ]; then
        log_message "SUCCESS: Processed $PROCESSED new orders"
    else
        log_message "INFO: No new orders to process"
    fi
    
    if [ "$ERROR_COUNT" -gt 0 ]; then
        log_message "WARNING: $ERROR_COUNT errors encountered during processing"
    fi
else
    log_message "ERROR: API request failed with status $HTTP_CODE"
    log_message "Response: $RESPONSE_BODY"
    exit 1
fi

log_message "Square order processing completed" 